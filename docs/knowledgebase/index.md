...
* [Architecture](concepts/architecture.md) - the horde node, coordinator/worker modes, and agent subprocess model.
...
* [Coordinator/worker cluster model](decisions/coordinator-worker-model.md) - distributed topology and its trade-offs.
* [Raft for leader election and coordinator-state replication](decisions/raft-leader-election.md) - hashicorp/raft for quorum-based election + coordinator-state replication via the raft log; the basis for leader failover.
...
* [Phase 2 — Server API](plans/phase-2-server-api.md) - node API transport, event streaming, worker↔coordinator contract.
...
* [Distributed project management](plans/distributed-project-management.md) - forward project mutations from worker to coordinator, and add a `horde project` CLI subcommand.
