---
type: Decision
title: Coordinator/worker cluster model
...
Adopt a coordinator/worker (leader/follower) topology:

* `coordinator` (default) is the central hub and source of truth.
* `worker` connects to a coordinator but is *not blocked* by that connection for
...
The `--mode` flag and the `mode` config key select the role. Workers without a
