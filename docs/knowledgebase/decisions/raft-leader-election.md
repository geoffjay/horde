---
type: Decision
title: Raft for leader election and coordinator-state replication
description: Adopt hashicorp/raft for quorum-based leader election and replicate coordinator-only state through the raft log, rather than a lease over gossip or manual standby promotion.
...
Through Phase 4 the coordinator is **statically designated** (`horde serve --mode
coordinator`) and is the single source of truth for cluster-wide state. Gossip
discovery (Phase 4 slice 5) makes the coordinator's address *findable* without static
config and the `Discoverer` re-resolves each reconnect, so workers already follow
a new leader **once one exists** — but nothing *elects* one, and nothing
preserves the coordinator's state across the change. The
...
(2) preservation of coordinator-only state, (3) re-targeting on leader change, and
(4) a stable advertised entry point.

The two coupled decisions — *how to elect* and *how coordinator state survives* — were
...
Adopt **`hashicorp/raft`** for leader election, and **replicate coordinator-only
state through the raft log** (project store, AAP resume tokens) via a raft FSM.

* The raft cluster's leader *is* the horde coordinator. A `raftDiscoverer` returns the
...
* Coordinator-only mutations (project create/assign/state, resume-token capture)
...
* The gossip ring is retained for *membership and failure detection*; raft is
  ...
  the static-coordinator deployments that do not enable failover.
...
* **Replicate via the log vs. rebuild-on-promotion:** rebuild is least work and
  ...
  store and resume tokens would be lost or need workers to re-report. Since raft
  already ships a replicated log, applying coordinator mutations through it is the
...
* **A ≥3-node quorum is required** for safe failover. A 2-node cluster cannot
  ...
  raft`), and the default single-coordinator topology is unchanged. Deployments that
...
* Coordinator-only mutation paths must be routed through the FSM (an internal
...
This extends the [coordinator/worker cluster model](coordinator-worker-model.md); see the
