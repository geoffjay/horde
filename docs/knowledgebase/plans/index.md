...
* [Knowledgebase sync](knowledgebase-sync.md) - planned (roadmap phase 6); share each project's `.horde/knowledgebase/` across the cluster, reaching **symmetric multi-writer** (every node's tree watched) in two stages — authority-serialized writes, content-digest identity, three-way convergence, and compare-and-swap. Opt-in; wire format in [KSP v1](/docs/spec/knowledgebase-sync-protocol-v1.md).
