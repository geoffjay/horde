...
| `knowledgebase.sync.push_user`     | *(empty)*           | `HORDE_KNOWLEDGEBASE_SYNC_PUSH_USER`     | horde user id a stage-2 watcher push is attributed to (echoed as `X-Horde-User`) when a local file edit has no originating API user. Must be a user with write authority on the synced projects (owner or admin) once per-user auth is enabled; empty ⇒ machine pushes carry no write identity and fail closed under auth (KSP §9). No effect when auth is disabled. |
